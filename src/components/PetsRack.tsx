...
function PetCard({ pet }: { pet: Pet }) {
  ...
  return (
    <div className={`pet-card pet-card--hatched ${tableColorClass(pet.table)}`}>
      <div
        className={`pet-dragon pet-dragon--lv${cool} ${tableColorClass(pet.table)}`}
      ...
      >
        <svg viewBox="0 0 140 80" className="pet-dragon-svg">
          ...
          <path
            className="pet-wing"
            d="M52 38 L28 8 L48 28 L40 4 L58 30"
            fill="currentColor"
            opacity="0.75"
          />
          {cool >= 3 && (
            <path
              className="pet-wing pet-wing--extra"
              d="M54 42 L34 18 L50 34"
              fill="currentColor"
              opacity="0.55"
            />
          )}
          <path
            className="pet-tail"
            d="M28 48 C12 42 4 52 2 62 C10 56 18 54 28 52"
            fill="currentColor"
          />
          <ellipse className="pet-torso" cx="68" cy="48" rx="28" ry="12" />
          <ellipse className="pet-head" cx="102" cy="36" rx="14" ry="11" />
          <polygon points="94,28 96,14 102,28" fill="currentColor" />
          <polygon points="102,26 106,12 112,26" fill="currentColor" />
          {cool >= 1 && <polygon points="108,24 114,8 118,26" fill="currentColor" />}
          {cool >= 4 && (
            <>
              <polygon points="88,34 84,22 92,32" fill="currentColor" />
